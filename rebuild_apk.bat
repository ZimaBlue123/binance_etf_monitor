...
rem ============================================================
rem  rebuild_apk.bat — Binance ETF Monitor APK 一键重建
rem
rem  用法:
rem    rebuild_apk.bat            自动递增版本号并构建 (默认,推荐)
rem    rebuild_apk.bat same       保持当前版本号不变
rem    rebuild_apk.bat check      仅检查环境与资产同步,不构建
rem    任意模式追加 nopause       结束后不暂停 (如 rebuild_apk.bat same nopause)
rem
rem  流程: 环境检查 -> 同步 assets -> MD5 校验 -> 版本递增
rem        -> 清理残留 -> Gradle 构建 -> 复制 APK 到根目录
rem ============================================================
...
if not exist "%JAVA_HOME%\bin\java.exe" (
    ...
    echo     请安装 JDK 17, 或设置 JAVA_HOME 环境变量后重试
    goto :fail
)
...
if not exist "%GRADLE%" (
    ...
    echo     请安装 Gradle 8.7 到 C:\Gradle\gradle-8.7, 或修改本脚本的 GRADLE 变量
    goto :fail
)
if not exist "%ANDROID%\keystore\keystore.properties" (
    echo [^!] 错误: 缺少签名配置 android\keystore\keystore.properties
    echo     请参考 android\BUILD.md 一次性生成 keystore 后重试
    goto :fail
)
...
if /i "%MODE%"=="check" (
    echo.
    echo [*] 模式=check: 环境检查通过, 未执行构建。
    echo     源配置   : %ROOT%config\
    echo     APK 资产 : %ASSETS%
    goto :end
)
...
if not "!SRC_MD5!"=="!ASSET_MD5!" (
    echo [^!] 错误: etf_products.json 同步不一致, 已中止
    echo     源文件   : !SRC_MD5!
    echo     APK 资产 : !ASSET_MD5!
    goto :fail
)
...
if not defined VN (
    echo [^!] 错误: 版本号处理失败, 请检查 android\app\build.gradle
    goto :fail
)
echo [OK] 新版本: v%VN%  (versionCode %VC%)

rem ---------- 4. 清理残留 ----------
echo.
echo [5/7] 停止 Gradle daemon 并清理上次构建残留...
...
rem ---------- 5. 构建 ----------
echo.
echo [6/7] 开始 Gradle 构建 (约 5-8 分钟, 请耐心等待)...
...
echo   文件 : %ROOT%BinanceETFMonitor-v%VN%-release.apk
echo ============================================================
goto :end
