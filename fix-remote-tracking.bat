...
REM
REM   修复本机 PortableGit + Windows 上偶尔出现的怪癖:
REM   git fetch 之后 refs/remotes/origin/main 写入失败, 导致
REM   git status 显示 [origin/main: gone]。
REM
REM   本脚本: fetch -> 检查 -> 必要时用 ls-remote 取 HEAD SHA
REM   -> mkdir + echo 重建 loose ref -> 验证。
REM
REM   用法: 双击 / 命令行 fix-remote-tracking.bat
REM ============================================================
...
REM 3. 不可用 -> 用 ls-remote 取 HEAD SHA + 重建
...
findstr /c:"\[gone\]" "%TEMP%\_wa_st2.txt" >nul 2>&1 && (
    ...
    echo   [OK] remote-tracking ref 已建立:
    type "%TEMP%\_wa_st2.txt"
)
